...
import { weekLoad } from '../data/plan'

type WeekLoadProps = {
  theme: 'light' | 'dark'
}

const keys = ['Platform', 'Member', 'AI', 'Clinician', 'Admin', 'Guardrails'] as const
...
export function WeekLoad({ theme }: WeekLoadProps) {
  ...
  const fills =
    theme === 'dark'
      ? ['#1c4f36', '#4a8a68', '#8fcbaa', '#b7dcc8', '#6ba584', '#24352c']
      : ['#143828', '#1c4f36', '#2f6a4a', '#4a8a68', '#6ba584', '#8fcbaa']

  return (
    <div className="h-[240px] w-full min-w-0 sm:h-[280px]">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={weekLoad} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
          <CartesianGrid stroke={grid} vertical={false} strokeDasharray="2 6" />
          <XAxis dataKey="week" tick={{ fill: tick, fontSize: 12, fontFamily: 'IBM Plex Mono' }} axisLine={false} tickLine={false} />
          <YAxis
            allowDecimals={false}
          ...
          />
          {keys.map((key, index) => (
            <Bar key={key} dataKey={key} stackId="load" fill={fills[index]} maxBarSize={42} />
          ))}
        </BarChart>
      </ResponsiveContainer>
    </div>
